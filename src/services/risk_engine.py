"""Deterministic Soft-Tissue Injury Risk Assessment Engine.

Evaluates physiological Z-scores, EWMA ACWR spikes, sleep deficits, and training strain
to output calibrated risk tiers, factor attributions, and clinical recommendations.
"""

from typing import List, Tuple
from src.domain.schemas import RiskAssessmentRead, RiskTier, TelemetryRead
from src.services.analytics import compute_workload_summary


def evaluate_risk(telemetry_history: List[TelemetryRead], target_sleep_hours: float = 8.0) -> RiskAssessmentRead:
    """Evaluates telemetry history and returns a deterministic RiskAssessmentRead DTO.

    Args:
        telemetry_history: Historical list of `TelemetryRead` entries (min 28 days).
        target_sleep_hours: Athlete target sleep baseline in hours.

    Returns:
        `RiskAssessmentRead` DTO.
    """
    sorted_history = sorted(telemetry_history, key=lambda t: t.recorded_date)
    latest = sorted_history[-1]
    workload = compute_workload_summary(sorted_history)

    # Compute 28-day baseline mean and std dev for HRV and RHR
